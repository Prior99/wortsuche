...
Wordsearch.prototype.initialize = function()
{
	...
	websocket.addHandler("game", function (param) {self.createArray(param);});
	websocket.addHandler("words", function (param) {self.setWords(param[0].split(","));});
	websocket.addHandler("remove", function (param) {self.removeWord(param[0], parseInt(param[1]), parseInt(param[2]), parseInt(param[3]), parseInt(param[4]));});
};

/*** Sets the list of all words to find. Parameter contains Array of all words ***/
Wordsearch.prototype.setWords = function(param)
{
	this.allWords = param;
	this.refreshWordDiv();
}
...
Wordsearch.prototype.removeWord = function(word, x1, y1, x2, y2)
{
	var ctx = this.ctx;
...
Wordsearch.prototype.redraw = function()
{
	...
	{
		this.drawLine(ctx, this.mouseXstart, this.mouseYstart, this.mouseXend, this.mouseYend, this.nextColor);
	}
};
