...
Wordsearch.prototype.initialize = function()
{
	...
	$('#send').keyup(function(e)
	{
		if(e.which == 13)
		{
			var val = replaceAll(replaceAll($('#send').val(),";","&sc&"),":","&dp&");
			websocket.runCommand("chat",[val]);
			$('#send').val("");
		}
	});
	websocket.addHandler("chat", function (param) 
	{
		var val = replaceAll(replaceAll(param[2],"&sc&",";"),"&dp&",":");
		chat(param[0], param[1], val, parseInt(param[3]));
	});
};
...
function chat(username, color, msg, time)
{
